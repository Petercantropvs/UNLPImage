...
#class PerfilVacio(Exception):
#    "El Usuario elgio crear perfil y no creó ninguno."
#    pass
...
def layout():
    ...
    col_1 = [[sg.Text('UNLPImage', font = ('latin modern sansquotation', 25))],
            [sg.Image(data=nuevoperfil.get_img_data(os.getcwd()+'/nuevo_perfil/prof_pictures/'+perfil+'.png', first = True), key='-PIC-', enable_events=True, metadata=0 ) ],
            [sg.Text('Editar Perfil', font = ('latin modern sansquotation', 15) )],
    ...
    return layout
def ventana_error():
    sg.popup_error('Estudiantes aún trabajando en esta funcionalidad, disculpe las molestias 👉👈', 
                       title = 'Falta modificar código de ventana!!',
                       image = err, icon = err, 
                       keep_on_top = True)
